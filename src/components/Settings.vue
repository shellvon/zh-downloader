    ...
    <el-main>
      <el-form label-position="left">
        <el-form-item label="偏爱的视频格式:">
          <el-select placeholder="请选择你偏好的视频格式" @change="onChange('format', $event)" :value="customSettings.format">
            ...
            <el-option label="MP4" value="mp4">
              ...
              <span :style="{ paddingLeft: '13px', float: 'right', color: 'red', fontSize: '12px' }"> 若数据源格式不是MP4,转换MP4可能存在兼容性问题,请慎用</span>
            </el-option>
          </el-select>
        ...
        <el-form-item label="视频过期时间:">
          <el-input-number controls-position="right" :value="customSettings.expiredAt" @change="onChange('expiredAt', $event)" :min="5" :max="240"></el-input-number>
        ...
        <el-form-item label="高级嗅探:">
          <el-switch v-model="advancedSniffer" :width="50"> </el-switch>
        ...
        <template v-if="advancedSniffer">
          <el-form-item label="嗅探配置(JSON):">
            <el-input
              type="textarea"
              placeholder="请输入高级嗅探配置"
              :value="customSettings.advancedSnifferConfig | toJSONStr"
              @change="onChange('advancedSnifferConfig', $event)"
            ></el-input>
